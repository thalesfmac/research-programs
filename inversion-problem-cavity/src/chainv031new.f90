...
program chain
    ...
    integer Lx,Nph,seed,distkind,Ndisorder,NEpoints
    real*8 t, gam, sigmaU, omega
    common /block1/ Lx,Nph,seed,distkind,t,gam,sigmaU,omega,Ndisorder,NEpoints
    ...
    read(5,*) outname
    read(5,*) Lx, Nph, seed, distkind
    read(5,*) NEpoints, Ndisorder
    read(5,*) t, gam, sigmaU, omega
...
!	 Energ= 2.d0*dfloat(ii-1)/dfloat(NEpoints)
...
!	 write(1015,*) Energ, G2(ii,nei)
            write(1015,*) Energ, bTt(ii,nei),logbTt(ii,nei)
        end do
    end do
...
!-------------------!
    ...
    write(1011,*) "Lx=",Lx, "Nph=", Nph, "seed=", seed
    write(1011,*) "Distribution (1 Gaussian, any other retangular)=",distkind
    write(1011,*) "Energy grid=", NEpoints, "Number of disorder conf.=", Ndisorder
    write(1011,*) "t=",t, "gam=", gam, "sigma (dist)=", sigmaU, "Omega=", omega
...
! INFO=0
    ...
    call randomdistribution(U,seed,distkind,Lx,sigmaU)
...
!%%%%%%%%%%%%% off-diagonal %%%%%%%%%%%%
! write(*,*) "Diagonalizando a matrix"
! call ZHEEV(JOBZ,UPLO,NN,H,LDA,lambda,WORK,LWORK,RWORK,INFO)
...
subroutine randomdistribution(U,seed,distkind,Lx,sigmaU)
    implicit none
    real*8 U(Lx),ran2, r1,r2,sigmaU,PI
    integer seed, distkind, Lx, i

    PI=dacos(-1.d0)

    if (distkind .eq. 1) then !Gaussian
        do i=1,Lx
            r1=ran2(seed)
            r2=ran2(seed)
            U(i) = sigmaU*dcos(2.d0*PI*r1)*dsqrt(-2.d0*dlog(1.d0-r2))
        end do
    else ! uniform
        do i=1,Lx
            U(i) = sigmaU*(ran2(seed) - 0.5d0)
        end do
    end if
