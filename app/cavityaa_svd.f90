program main
   ...
   use stdlib_linalg, only: eigvalsh, svdvals, diag
   ...
   complex(dp), allocatable :: H(:, :), Heff(:, :), M(:, :), Id(:, :)
   real(dp), allocatable :: eig(:), w(:, :), d(:, :), sv(:, :, :)
   ...
   allocate (sv(L*(Nph + 1), NEpoints, Ndisorder))
   ...
   do j = 1, Ndisorder
      ...
      do i = 1, NEpoints
         ...
         Heff = H
         Heff(1, 1) = Heff(1, 1) + surface_gf_1d(energies(i), tlead, muS)
         Heff(NN, NN) = Heff(NN, NN) + surface_gf_1d(energies(i), tlead, muD)

         M = energies(i)*Id - Heff
   ...
   call save_npy("sv_"//trim(outname)//".npy", sv)
